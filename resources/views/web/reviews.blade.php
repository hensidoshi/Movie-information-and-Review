            ...
            @foreach($movies as $movie)
                <div class="movie-card">
                    ...
                    <div class="reviews-box">
                        ...
                        @auth
                            <form method="POST" action="{{ url('/review/store/'.$movie->slug) }}">
                                @csrf
                                <div class="star-rating">
                                    @for ($i = 5; $i >= 1; $i--)
                                        <input type="radio" id="star{{ $i }}-{{ $movie->id }}" name="rating" value="{{ $i }}" required />
                                        <label for="star{{ $i }}-{{ $movie->id }}" title="{{ $i }} stars">
